...
vi.mock("../src/web/useLocalCommand.js", () => ({
  useLocalCommand: () => ({ execute: vi.fn(), pending: false, error: null, notice: null }),
}));
...
it("clears old artwork immediately on disabled status and never renders lyrics or timing", async () => {
  const fetcher = service();
  await act(async () => { render(<LineInAlbumView />); });
  expect(screen.getByText("Recognized album")).toBeInTheDocument();
  fetcher.mockResolvedValue({ ok: true, json: async () => ({
    state: "disabled", expiresAt: Date.now() + 4000, key, album: null,
  }) });
  await act(async () => vi.advanceTimersByTimeAsync(800));
  expect(screen.queryByText("Recognized album")).not.toBeInTheDocument();
  expect(screen.queryByRole("img")).not.toBeInTheDocument();
  expect(screen.getAllByText("Album recognition is off").length).toBeGreaterThan(0);
});

it("expires album locally even if the next status request never completes", async () => {
  const fetcher = service();
  await act(async () => { render(<LineInAlbumView />); });
...
it("keeps identified album artwork/name when the catalog is incomplete and clears everything on disable", async () => {
